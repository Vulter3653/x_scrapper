...
def validate_queue_validator() -> None:
    ...
    result = subprocess.run(
        [sys.executable, str(QUEUE_VALIDATOR)],
        cwd=REPO_ROOT,
        text=True,
        capture_output=True,
        check=False,
    ...
    if result.returncode != 0:
        ...
        report("PASS", "queue validator", "validated source queue before readiness checks")
...
def validate_policy() -> None:
    ...
    if actual_count != expected_count:
        report("FAIL", "eligible account count", f"policy has {actual_count}, queue has {expected_count}")
    else:
        report("PASS", "eligible account count", f"policy matches queue row count: {expected_count}")
    ...
    if found:
        ...
        report("PASS", "readiness boundary wording", "policy does not imply scraping, MCP, API, or complete historical coverage")

    doc_text = DOC_FILE.read_text(encoding="utf-8")
    required_doc_phrases = [
        "dry-run readiness protocol only",
        "does not authorize scraping",
        "does not authorize MCP installation",
        "does not authorize X API usage",
        "does not authorize dashboard sync",
        "does not authorize `data/` or `dashboard/data/` mutation",
        "limited to retrievable timeline posts",
